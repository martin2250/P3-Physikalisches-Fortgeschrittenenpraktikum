...
Few of them are
\begin{itemize}
	\item \textbf{Mass of thingmajigs/Stiffness of springs:} Only the mean mass of all thingmajigs is given, so it is not possible to make assumptions regarding how much the individual thingmajigs vary from their mean value.
	The same problem applies for the stiffness of the used springs. All springs were assumed to be identical, which is practically impossible.
	\item \textbf{Video measurement} As described in \autoref{sec:data_acq}, a camera with a horizontal, linear CCD is used to capture the motion of the thingmajigs.
	This is not an optimal solution, since environmental influences, such as unwanted reflections, may alter measurements.
	Furthermore, no information concerning the spatial resolution of the camera is given, so it cannot be accounted for.
\end{itemize}

...
\begin{figure}
	...
	Found fit parameters: $D_\text{opt}=\SI{27.38(15)}{\newton\per\meter}, D_\text{ac}=\SI{23.89(54)}{\newton\per\meter}$. Goodness of fit: $\chi_\text{opt, red}=\num{0.006}, \chi_\text{ac, red}=\num{0.022}$.}
...
\begin{align*}
	...
	\left\{ \begin{array}{@{}ll@{}}
		D_\text{bi, opt} = \SI{27.38(15)}{\newton\per\meter} \\
		D_\text{bi, ac} = \SI{23.89(54)}{\newton\per\meter}
	\end{array}\right\} \Rightarrow \bar{D}_\text{bi} &= \left(\num{25.64}\pm \num{1.23}\ \text{(std)}\pm \num{0.28}\ \text{(stat)}\right)\si{\newton\meter^{-1}}.
\end{align*}
...
\begin{alignat*}{3}
	...
	D_\text{bi} &= \frac{2(m+M)\cdot v_\text{bi}^2}{a^2} &&= \SI{22.25(18)}{\newton\meter^{-1}}. \\
\end{alignat*}
As expected, the stiffness for both springs is the same.
...
The amplitude ratios are plotted in \autoref{fig:amplitude-ratio} for both acoustic and optic modes.
A least-squares fit of the theoretical model \autoref{eq:massratio} yields the parameters $\gamma = \frac{M}{m} = \num{1.39(2)}$ for the acoustic branch and $\frac{M}{m} = \num{1.46(2)}$ for the optic branch.

...
The ratio of amplitudes $\frac{U}{V}$ can also be determined as
\begin{equation}\label{eq:massratio}
	\frac{U}{V} = \frac{\cos\left(k a\right)}{1 - \frac{1 + \gamma}{2 \gamma} \left[1 \pm \sqrt{1 - \frac{4 \gamma}{\left(1 + \gamma\right)^2} \sin^2\left(k a\right)}\right]}
\end{equation}
with the mass ratio $\gamma = \frac{m_2}{m_1}$.

In a finite chain, the correct solution is a superposition of two waves with identical amplitudes travelling in opposite directions.
This ensures that the boundary conditions of fixed ends are met.
As a result, the amplitude of individual elements with identical weights in a finite chain also depend on the element's position.
This dependence can be expressed by an envelope function proportional to $\sin\left(\frac{\uppi x n}{L}\right)$ where $x$ is the position in the chain and $n$ is the number of the excited mode.
The envelope function is identical for both kinds of elements.
